...
with open('RSA_1_layer_loss.txt','w') as f:
    f.write(str(result1.history))

model.save('RSA_1_layer_loss.h5')
...
with open('RSA_2_layers_loss.txt','w') as f:
    f.write(str(result2.history))

model.save('RSA_2_layers_loss.h5')


plt.grid(ls='--')
plt.xlabel('Number of Epochs')
plt.ylabel('Loss')
plt.plot(result1.epoch, result1.history['loss'], 'red', lw=2,  label = '1 LSTM layer')
plt.plot(result2.epoch, result2.history['loss'], 'green', lw=2, label = '2 LSTM layers')

plt.legend()
plt.savefig("RSA_nlayers_loss_result.eps", format='eps', dpi=1000)
plt.show()
